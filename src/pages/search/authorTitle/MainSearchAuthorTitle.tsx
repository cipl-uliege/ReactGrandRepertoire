...
import SkeletonLoaderAuthorTitle from "./SkeletonLoaderAuthorTitle";
...
export default function MainSearchAuthorTitle() {
    ...
    const [resultFromAPI, setResultFromAPI] = useState<IPaginatedResultsAPI<ISheetMusicFromAuthorTitleAPI[]> | null>(null);
    ...
    const refreshUrlWithCorrectParameters = () => {
        navigate(`../author_or_title?&results_per_page=${selectedResultsPerPage}&page=${page}&author=${author.current}&title=${title.current}`, { replace: true });
    }
    ...
    const callbackWhenFetched = (json: IPaginatedResultsAPI<ISheetMusicFromAuthorTitleAPI[]>) => {
    ...
    return (
        <div className="max-w-5xl mx-auto">
            ...
            <h2 className="font-raleway text-center text-xl uppercase font-black underline underline-offset-4 mt-2">
                Recherche par auteur, titre ou nombre d'instrumentistes
            </h2>
                    ...
                    <div className="max-w-full sm:w-[720px] sm:max-w-none px-2">
                        {
                            isRequestPending &&
                            <>
                                <SkeletonLoaderAuthorTitle
                                    author={author.current}
                                    title={title.current}
                                    nbrInstrumentalists={nbrInstrumentalists.current}
                                    callbackWhenFetched={callbackWhenFetched}
                                    page={page}
                                    resultsPerPage={selectedResultsPerPage}></SkeletonLoaderAuthorTitle>
                            </>
                        }
                        {
