...
export default function SkeletonLoaderSheetMusicFavorite(props: { 
    resultsPerPage: number,
    page: number,
    author:string,
    title:string,
    nbrInstrumentalists:number
    callbackWhenFetched: (paginatedData : IPaginatedResultsAPI<IFavoriteSheetMusicAPI[]>) => void,
}) {

    useEffect(() => {
        let isDataFetched = false;
        let abortController = new AbortController();
        Post.getFavoriteSheetMusic(abortController.signal, props.page, props.resultsPerPage, props.author, props.title, props.nbrInstrumentalists)
            .then(json => {
                isDataFetched = true;
