...
import SkeletonLoaderSheetMusicGroupId from "./SkeletonLoaderSheetMusicGroupId";
...
export default function MainSheetMusicGroupId() {
    ...
    useEffect(() => {
        ...
        fetchNewResult();
        // document.getElementById("listSheetMusics")?.scrollIntoView({ behavior: "smooth", block: "start" });
    ...
    const callbackWhenFetched = (json: IPaginatedResultsAPI<ISheetMusicFromGroupAPI[]>) => {
                ...
                <div id="listSheetMusics" className="max-w-4xl mx-auto flex flex-col sm:justify-center sm:flex-row-reverse relative mt-4">
                    ...
                    <div className="max-w-full sm:w-[720px] sm:max-w-none px-2">
...

                        {
                            isRequestPending &&
                            <>
                                <SkeletonLoaderSheetMusicGroupId
                                    author={author.current}
                                    title={title.current}
                                    groupId={groupId}
                                    callbackWhenFetched={callbackWhenFetched}
                                    page={page}
                                    resultsPerPage={selectedResultsPerPage}></SkeletonLoaderSheetMusicGroupId>
                            </>
                        }
                        {
        ...
        <>
            {
                ...
                <div className="flex items-center">
                    ...
                    <h2 id="compositionName" className="px-1 font-raleway text-blue-950 font-black text-3xl text-center sm:text-left">Partition(s) pour <i>{props.composition}</i></h2>
                </div>
            }
            {
                props.composition != null && props.composition == "" &&
                <h2 className="px-1 font-raleway text-red-700 font-black text-xl text-center sm:text-left">Aucune formation n'existe avec l'identifiant passé</h2>
            }
        </>
    )
}
    ...
    return (
        <div className="bg-white shadow-md sm:shadow-none flex flex-col">
            ...
            <div className="px-2 pb-2">
                <h3 className="font-black text-center uppercase sm:mt-3">{props.totalNumberOfResults} partition(s) trouvée(s)</h3>
            </div>
